...
module File =
    let writeAllLines (path: string) (lines: string seq) = File.WriteAllLines(path, lines)

    let writeAllText (path: string) (contents: string) = File.WriteAllText(path, contents)
